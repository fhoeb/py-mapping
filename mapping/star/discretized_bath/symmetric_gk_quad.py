...
from mapping.utils.integration_defaults import default_epsrel, default_epsabs
import numpy as np


class GKQuadDiscretizedSymmetricBath(BaseDiscretizedSymmetricBath):
    def __init__(self, J, domain, max_nof_coefficients=100, interval_type='lin', **kwargs):
        ...
        self.comp_gamma_buf = np.empty(2 * max_nof_coefficients)
        self.comp_xi_buf = np.empty(2 * max_nof_coefficients)
    ...
    def compute_coefficients(self, stop_n):
        ...
        if isinstance(self.dx_m, np.ndarray):
            ...
            dx_m_view = self.dx_m
        compute_gk_quadrature_integrals(self.J, mid_p[self._next_n:stop_n],
                                        dx_p_view,
                                        self.comp_gamma_buf[buffer_start:buffer_stop_pos],
                                        self.gamma_buf[buffer_start:buffer_stop_pos],
                                        self.comp_xi_buf[buffer_start:buffer_stop_pos],
                                        self.xi_buf[buffer_start:buffer_stop_pos],
                                        ignore_zeros=self.ignore_zeros, epsabs=self.epsabs, epsrel=self.epsrel)
        compute_gk_quadrature_integrals(self.J, self.mid_m[self._next_n:stop_n],
                                        dx_m_view,
                                        self.comp_gamma_buf[buffer_stop_pos:buffer_stop],
                                        self.gamma_buf[buffer_stop_pos:buffer_stop],
                                        self.comp_xi_buf[buffer_stop_pos:buffer_stop],
